...
public class CommandPacketListener implements PacketListener {
	...
	private static void devCommands(Player player, String command, String[] parts) {
		...
		if(parts[0].startsWith("save")) {
			player.getPacketSender().sendMessage("Saved");
			player.save();
		}
		...
		if (parts[0].equals("test")) {
            player.getPacketSender().sendItemOnInterface(18500, 4151, 1);
            player.getPacketSender().sendInterface(42000);
		} 
		if (parts[0].equals("find")) {
			String name = command.substring(parts[0].length() + 1);
			boolean found = false;
			for (int i = 0; i < ItemDefinition.DEFINITIONS.length; i++) {
				if (ItemDefinition.forId(i).getName().toLowerCase().contains(name)) {
					player.getPacketSender().sendItemOnInterface(18500, i, 1);
					player.getPacketSender().sendInterface(42000);
			...
			if (!found) {
				player.getPacketSender().sendMessage("[" + name + "] can not be found.");
			}
		}
	}
